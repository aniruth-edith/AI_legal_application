...
#####################################################################################################################################
# import torch
# # REMOVED: import torch_sparse  -- replaced with pure PyTorch implementation
# from tqdm import tqdm
# from collections import defaultdict
...
# # ── Pure PyTorch SparseTensor replacement for torch_sparse.SparseTensor ───────

# class SparseTensor:
#     """
#     Drop-in replacement for torch_sparse.SparseTensor.
#     Only implements .sample() which is the only method used by LeSICiN's
#     MetapathAggrNet in submodules.py.
#     """
#     def __init__(self, row, col, sparse_sizes):
#         self.sparse_sizes = sparse_sizes
#         # Build adjacency list for fast neighbor sampling
#         self._adj = defaultdict(list)
#         for r, c in zip(row.tolist(), col.tolist()):
#             self._adj[r].append(c)
#         # Pre-convert to tensors
#         self._adj_tensors = {
#             src: torch.tensor(neighbors, dtype=torch.long)
#             for src, neighbors in self._adj.items()
#         }

#     def sample(self, num_neighbors: int, subset: torch.Tensor) -> torch.Tensor:
#         """
#         For each node index in subset, randomly sample num_neighbors neighbors.
#         Returns shape: [len(subset), num_neighbors]
#         Falls back to self-loop if a node has no neighbors.
#         """
#         results = []
#         for node in subset.tolist():
#             neighbors = self._adj_tensors.get(int(node))
#             if neighbors is None or len(neighbors) == 0:
#                 sampled = torch.full((num_neighbors,), int(node), dtype=torch.long)
#             else:
#                 idx = torch.randint(0, len(neighbors), (num_neighbors,))
#                 sampled = neighbors[idx]
#             results.append(sampled)
#         return torch.stack(results, dim=0)  # [D, num_neighbors]
...
#             train_data.dataset + label_data.dataset,
...
# def generate_graph(label_vocab, type_map, label_tree_edges,
#                    cit_net_edges, label_name='section'):
#     # each key is a node_type, value is a dict of node_id -> index
#     node_vocab = defaultdict(dict)
#     node_vocab[label_name] = label_vocab  # keep label vocab consistent

#     edge_vocab = {}
#     edge_indices = defaultdict(list)

#     for (node_a, edge_type, node_b) in label_tree_edges + cit_net_edges:
#         node_a_type = type_map[node_a]
#         node_b_type = type_map[node_b]
...
#         node_a_token = node_vocab[node_a_type][node_a]
#         node_b_token = node_vocab[node_b_type][node_b]
#         edge_indices[(node_a_type, edge_type, node_b_type)].append(
#             [node_a_token, node_b_token]
#         )

#     num_nodes = {ntype: len(nodes) for ntype, nodes in node_vocab.items()}

#     # Build adjacency using pure PyTorch SparseTensor (no torch_sparse)
#     adjacency = {}
#     for keys, edges in edge_indices.items():
#         row, col = torch.tensor(edges).t()
#         sizes = (num_nodes[keys[0]], num_nodes[keys[-1]])
#         adjacency[tuple(keys)] = SparseTensor(
#             row=row, col=col, sparse_sizes=sizes
#         )
...
# def generate_label_weights(train_data, label_vocab,
#                            dev='cuda:0', scheme="tws", thresh=10.):
#     pos = torch.zeros(len(label_vocab), device=dev)
#     for instance in tqdm(train_data, desc="Generating label weights"):
#         for l in instance['labels']:
#             pos[label_vocab[l]] += 1
#     if scheme == 'tws':
#         weights = torch.clamp(pos.max() / pos, max=thresh)
#     else:
#         weights = len(train_data) / pos
#     return weights


# # ── Training / evaluation / inference pass ────────────────────────────────────

# def train_dev_pass(model, optimizer, fact_loader, sec_batch,
#                    metrics=None, pred_threshold=None,
#                    train=False, infer=False, label_vocab=False):
#     model.train() if train else model.eval()
...
#     for i, fact_batch in enumerate(
#         tqdm(fact_loader, desc="Flowing data through model")
#     ):
#         torch.cuda.empty_cache()

#         loss, predictions = model(
#             fact_batch.cuda(), sec_batch.cuda(), pthresh=pred_threshold
#         )
...
#             metrics(predictions, fact_batch.labels, loss=batch_loss)
#         else:
#             for j, instance_preds in enumerate(predictions):
#                 pred_list_indices = torch.nonzero(
#                     instance_preds, as_tuple=False
#                 ).squeeze(1)
#                 pred_list = [inv_label_vocab[idx.item()]
#                              for idx in pred_list_indices]
#                 outputs.append({
#                     'id': fact_batch.example_ids[j],
...
#         self.predictions += predictions.sum(dim=0)
#         self.labels      += labels.sum(dim=0)
#         self.run_jacc    += torch.sum(
#             torch.logical_and(predictions, labels).sum(dim=1) /
#             torch.logical_or(predictions, labels).sum(dim=1)
#         ).item()
#         self.counter += 1
#         if loss is not None:
#             self.run_loss += loss
...
#         self.micro_rec  = match_total / labels_total
#         self.micro_f1   = (
#             0 if self.micro_prec + self.micro_rec == 0
#             else 2 * self.micro_prec * self.micro_rec /
#                  (self.micro_prec + self.micro_rec)
#         )
#         self.jacc = self.run_jacc / self.counter
#         if 'run_loss' in self.__dict__:
#             self.loss = self.run_loss / self.counter
