...
# ── sent2vec replacement (Windows-compatible, no C++ build needed) ─────────────
# REMOVED: import sent2vec
# REMOVED: sent2vec_model = sent2vec.Sent2vecModel()
from sent2vec_adapter import Sent2vecModel
sent2vec_module = type('sent2vec', (), {'Sent2vecModel': Sent2vecModel})()
# Now use: sent2vec_module.Sent2vecModel() instead of sent2vec.Sent2vecModel()

from model.model import LeSICiN
from data_helper import LSIDataset, collate_func
from helper import (
    generate_vocabs, generate_graph, generate_label_weights,
    train_dev_pass, MultiLabelMetrics
)
...
# ── Load sent2vec adapter (replaces sent2vec.Sent2vecModel) ───────────────────
if dc['s2v_path'] is not None:
    sent2vec_model = Sent2vecModel()
    sent2vec_model.load_model(dc['s2v_path'])  # path is ignored by adapter
else:
    sent2vec_model = Sent2vecModel()
...
# For inference-only mode, use sec_dataset as stand-in for train_dataset
# since generate_vocabs only needs sent_vectorized flag from train_data
if hc['do_train_dev']:
    vocab, label_vocab = generate_vocabs(
        train_dataset, sec_dataset,
        limit=hc['vocab_limit'], thresh=hc['vocab_thresh']
    )
else:
    # Inference mode: no train dataset needed
    vocab, label_vocab = generate_vocabs(
...
node_vocab, edge_vocab, edge_indices, adjacency = generate_graph(
    label_vocab, type_map, label_tree, citation_net
)

# Only generate label weights if training
if hc['do_train_dev']:
    sec_weights = generate_label_weights(train_dataset, label_vocab)
else:
    sec_weights = None
...
# ── Section dataloader (static — used in every forward pass) ──────────────────
sec_loader = torch.utils.data.DataLoader(
    sec_dataset,
    batch_size=len(label_vocab),
    collate_fn=partial(
    ...
    num_workers=0,  # 0 workers required on Windows
)
...
# Pre-fetch the section batch once (static across all forward passes)
for sec_batch in sec_loader:
    break
...
if device == 'cuda':
    lsc_model = lsc_model.cuda()

if dc['model_load'] is not None:
    lsc_model.load_state_dict(
        torch.load(dc['model_load'], map_location=device)
    )
    print(f"Loaded model weights from: {dc['model_load']}")

# ── Train / Dev ────────────────────────────────────────────────────────────────
if hc['do_train_dev']:
    if dc['metrics_load'] is not None:
        with open(dc['metrics_load'], 'rb') as fr:
            best_metrics = pkl.load(fr)
            best_loss = best_metrics.loss
    else:
        best_loss = float('inf')
    ...
    optimizer = torch.optim.AdamW(
        lsc_model.parameters(),
        lr=hc['opt_lr'],
        weight_decay=hc['opt_wt_decay']
    )
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min',
        patience=hc['sch_patience'],
        factor=hc['sch_factor']
    )
    train_mlmetrics = MultiLabelMetrics(L)
    dev_mlmetrics   = MultiLabelMetrics(L)
    ...
    print("==================================================")
    for epoch in range(hc['num_epochs']):
        train_mlmetrics = train_dev_pass(
            lsc_model, optimizer, train_loader, sec_batch,
            metrics=train_mlmetrics, train=True, pred_threshold=hc['pthresh']
        )
        dev_mlmetrics = train_dev_pass(
            lsc_model, optimizer, dev_loader, sec_batch,
            metrics=dev_mlmetrics, pred_threshold=hc['pthresh']
        )

        train_loss = train_mlmetrics.loss
        dev_loss   = dev_mlmetrics.loss
        ...
        print("%5d || %.4f | %.4f || %.4f | %.4f %.4f %.4f" % (
            epoch,
            train_loss, train_mlmetrics.macro_f1,
            dev_loss, dev_mlmetrics.macro_prec,
            dev_mlmetrics.macro_rec, dev_mlmetrics.macro_f1
        ))
    ...
    print("VALIDATION Results || %.4f | %.4f %.4f %.4f" % (
        best_loss,
        best_metrics.macro_prec, best_metrics.macro_rec, best_metrics.macro_f1
    ))
...
if hc['do_test']:
    ...
    print("\nRunning Test")
    print("==================================================")
    test_mlmetrics = train_dev_pass(
        lsc_model, optimizer, test_loader, sec_batch,
        metrics=test_mlmetrics, pred_threshold=hc['pthresh']
    )
    with open(dc['test_metrics_dump'], 'wb') as fw:
        pkl.dump(test_mlmetrics, fw)
    print("TEST Results || %.4f | %.4f %.4f %.4f" % (
        test_mlmetrics.loss,
        test_mlmetrics.macro_prec, test_mlmetrics.macro_rec, test_mlmetrics.macro_f1
    ))
...
if hc['do_infer']:
    ...
    infer_outputs = train_dev_pass(
        lsc_model, None, infer_loader, sec_batch,
        infer=True, pred_threshold=hc['pthresh'], label_vocab=label_vocab
    )
    with open(dc['infer_trg'], 'w') as fw:
        fw.write('\n'.join([json.dumps(doc) for doc in infer_outputs]))
    print(f"Inference complete. {len(infer_outputs)} documents processed.")
    print(f"Results saved to: {dc['infer_trg']}")